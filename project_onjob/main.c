...
{
    printf("Getting PID for process: %s\n", process_name);
    char command[50] = {0};
...
mem_info_t get_heap(pid_t pid)
{
    ...
    {
        if(strstr(buff_heapsmaps, "[heap]") != NULL)
        {
            //This is heap memory
            heap_point = true;
...
char *get_libc_path(pid_t pid)
{
    ...
    while(fgets(line, BUFFER_SIZE, maps_file) != NULL)
    {
        ...
        {
            char *path_start = strchr(line, '/');  /* return a pointer to the first of '/' but path_start is pointing to the "line" buffer, 
                                                    it is a ring buffer, discard when function terminate*/
            if(path_start != NULL)
            {
...
void cat_ltrace_log(pid_t pid)
{
    char buff_ltrace[BUFFER_SIZE];
    sprintf(buff_ltrace, "./ltrace_%d.log", pid);
    FILE *ltrace_file = fopen(buff_ltrace, "r");
    if(ltrace_file == NULL)
    {
        handle_err("ltrace_file fopen()");
    }

    char line[BUFFER_SIZE];
    while(fgets(line, BUFFER_SIZE, ltrace_file) != NULL)
    {
        printf("%s", line);
    }
}
...
void *monitor_process(void *args)
{
    ...
    for(;;)
    {
        ...
        printf("------------------------------------------------------------------------\n");
        cat_ltrace_log(process_info->pid);
        //get_heap_free_bytes(process_info->pid, "./output.txt");
...
int main(int argc, char *argv[])
{
    ...
    for(i = 0; i < process_number; i++)
    {
        ...
        if(return_value == 0)
        {
            ...
            char path_log_str[BUFFER_SIZE];

            sprintf(pid_str,"%d", process_info[i].pid);
            sprintf(path_log_str, "./ltrace_%d.log", process_info[i].pid);

            if(execl("/usr/bin/ltrace","ltrace", "-p", pid_str, "-e", "malloc+free", "-o", path_log_str, NULL) == -1)
            {
                handle_err("execl");
            }
